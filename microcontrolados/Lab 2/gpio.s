...
; 7. Para habilitar resistor de pull-up interno, setar PUR para 1
	...
	MOV     R1, #2_00000011						
	STR     R1, [R0]
	LDR     R0, =GPIO_PORTM_AHB_PUR_R			
	MOV     R1, #2_11110000						
	STR     R1, [R0]
	
	BX LR
	
PortL_Output ;colunas do teclado 
...
PortM_Input ;linhas do teclado

	LDR R2, =GPIO_PORTM_AHB_DATA_R
	LDR R0, [R2] ;vai ler o botão apertado e salvar em R0
	ORR R0, #2_00001111
...
LCD_Comand ;Instruções para o LCD
	
	PUSH {LR}
	
	LDR R2, =GPIO_PORTK_AHB_DATA_R 
	STR R1, [R2] ;barramento de dados (porta K) recebe o dado presente em R1
	
	MOV R0, #2
	BL SysTick_Wait1ms
	
	MOV R1, #2_00000100			;Ligando enable (RS = 0, RW = 0, E = 1)
	LDR R2, =GPIO_PORTM_AHB_DATA_R
	STR R1, [R2]
	...
	MOV R1, #2_00000000				;Desligando enable (PM2 = 0)
	LDR R2, =GPIO_PORTM_AHB_DATA_R
	STR R1, [R2]
	
	POP {PC}

LCD_Print ;Escrita no LCD
	...
	MOV R1, #2_00000101			;Ligando enable (RS = 1, RW = 0, E = 1)
	LDR R2, =GPIO_PORTM_AHB_DATA_R
	STR R1, [R2]
