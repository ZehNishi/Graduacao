...
// Versao 1.0 -- Março de 2015

// Programa de teste da implementaçao de fila genérica queue.c/queue.h.

// operating system check
//#if defined(_WIN32) || (!defined(__unix__) && !defined(__unix) && (!defined(__APPLE__) || !defined(__MACH__)))
//#warning Este codigo foi planejado para ambientes UNIX (LInux, *BSD, MacOS). A compilaçao e execuçao em outros ambientes é responsabilidade do usuário.
//#endif
...
// de ambas as estruturas forem os mesmos. De acordo com a seçao 6.7.2.1 do
// padrao C99: "Within a structure object, the non-bit-ﬁeld members and the
...
// imprime na tela um elemento da fila (chamada pela funçao queue_print)
...
// retorna 1 se a estrutura da fila esta correta, 0 senao
int fila_correta (filaint_t *fila)
{
   ...
   do
   {
      // testa ponteiro next (avaliaçao em curto-circuito)
      ...
      // testa ponteiro prev (avaliaçao em curto-circuito)
...
void testafila()
{
   ...
   do
   {
      assert (i == aux->id) ; // testa posiçao do elemento i
      i++ ;
      aux = aux->next ;
   } while (aux != fila0) ;
...
//}
   ...
   UARTprintf ("Remocao %d vezes o ultimo elemento...\n", N) ;
   i=0 ;
   while (i<N)
   {
      aux = fila0->prev ;
   ...
   // remocoes aleatorias
   UARTprintf ("Remocao %d vezes um elemento aleatorio...\n", N) ;
   while (fila0)
   {
   ...
   UARTprintf ("Testando remocao de elemento que esta em outra fila...\n") ;
   ...
   UARTprintf ("Testando remocao de elemento que nao esta em nenhuma fila...\n") ;
   ...
   UARTprintf ("Ok, nao deixou remover um elemento que nao esta em nenhuma fila\n") ;

   // tentar inserir algo que ja esta na mesma fila
   UARTprintf ("Testando insercao de elemento que ja esta na fila...\n") ;
   queue_append ((queue_t**) &fila0, (queue_t*) &item[0]) ;
   assert (fila0 == &item[0]) ;
   assert (item[0].prev == &item[0]) ;
   assert (item[0].next == &item[0]) ;
   UARTprintf ("Ok, nao deixou inserir elemento que ja estava na fila\n") ;

   // tentar inserir algo que esta em outra fila
   UARTprintf ("Testando insercao de elemento que esta em outra fila...\n") ;
   ...
   assert (item[1].prev == &item[1]) ;
   assert (item[1].next == &item[1]) ;
   UARTprintf ("Ok, nao deixou inserir elemento que esta em outra fila\n") ;
   ...
   UARTprintf ("Testes de operacoes invalidas funcionaram!\n") ;
